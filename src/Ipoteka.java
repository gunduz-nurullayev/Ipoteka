    ...
    public void hesabat(BigDecimal aylarinSayi, BigDecimal odenileBilenMebleg, BigDecimal umumiSumma) {
        ...
        if (odenileBilenMebleg.compareTo(ayliqOdenis) >= 0) {
            for (int i = 1; i <= aylarinSayi.intValue(); i++) {
                ...
                users.faiz = faiz.setScale(2, BigDecimal.ROUND_HALF_EVEN);
                users.ayliqOdenis = ayliqOdenis.setScale(2, BigDecimal.ROUND_HALF_EVEN);
                users.date = odenisTarixi;
                users.esasMebleg = esasMebleg.setScale(2, BigDecimal.ROUND_HALF_EVEN);
                users.qaliqBorc = qaliqBorc.setScale(2, BigDecimal.ROUND_HALF_EVEN);
                usersList.add(users);
            }
